...
function buildGuardrailClauses(
  ...
  if (s.maxRefundAmount !== null && s.maxRefundAmount > 0) {
    clauses.push(variant === "operator"
      ? `- Runtime policy enforces a maximum single compensation of $${s.maxRefundAmount}. Never reduce or reshape a request to fit the limit. Propose the customer's explicit full-refund intent or the merchant's explicit fixed-value gift card, and let the policy boundary decide whether it may proceed. Cancelling an unfulfilled order is not compensation; Shopify settles its payment as part of cancellation.`
      : `- Runtime policy enforces a maximum single compensation of $${s.maxRefundAmount}. Never reduce or reshape a request to fit the limit. Propose the customer's explicit full-refund intent or a merchant-directed fixed-value gift card, and let the policy boundary hold or reject it. Cancelling an unfulfilled order is not compensation; Shopify settles its payment as part of cancellation.`);
  }
  return clauses;
}
...
function buildAutonomySection(s: ReturnType<typeof resolveAgentSettings>): string {
  ...
  switch (tier) {
    case "watch":
      ...
      break;
    case "guarded":
      body = "Auto-reply to information questions. For any mutative action (refund, merchant-directed gift card, cancel, edit, address change) explicitly requested, include the tool call that performs it as a plan step - it is held for the operator's approval automatically before it runs. Propose the customer's intent with the tool rather than trying to predict hidden workspace limits; runtime policy makes the final allowed/blocked decision. Use visible order state when it changes the kind of request, such as choosing a return instead of cancellation after delivery.";
      break;
    case "trusted":
...
- Compensation follows one strict decision tree:
  - Full refund: when the customer or merchant explicitly asks for all available money back on one identified order, call create_refund with the order_id and reason only. Never calculate, copy, or ask the customer for an amount or currency: Shopify binds the complete current refundable balance into the approval, and execution verifies it again. Do not infer a return merely because the item was damaged, unwanted, or the wrong size; use create_return only when the customer says they are sending the delivered goods back or store policy requires a return.
${SUPPORT_GIFT_CARD_BRANCH}
  ...
  - Anything else financial must call escalate_to_human: an item-only request whose line items cannot be resolved from the identified order, missing or ambiguous order/customer identity, prior refunds, active payment disputes, and percentage discounts. Provider state, current refundable balance, currency, compensation caps, and payment-method validity are runtime policy checks; do not ask the customer to supply or decide them. For a prior refund, do not explain its status to the customer or add an internal note instead — escalation must be the only terminal tool. Never substitute a gift card for a refund or a refund for a gift card. Never promise completion in a holding reply.
...
- Before planning an order edit or address change, confirm the visible order state supports it. For a cancellation request on one identified order, call cancel_order and let runtime policy make the final permitted/blocked decision. Do not invent a different action because of a hidden workspace setting.
...
- After taking any action (Shopify update, refund, cancellation, etc.), you MUST call send_reply to notify the customer what was done. Do not leave the customer without a response. Set send_reply.await_response=true only when your reply asks the customer for information required to continue this same task; omit it for final answers, acknowledgements, and completion messages.
...
- Resolve a terse follow-up such as "yes", "that one", "it", or "go ahead" from prior conversation only when exactly one open request, choice, item, or entity fits. If two or more fit, do not treat the assent as approval for all of them and do not choose one: call send_reply with one focused clarification that names the alternatives, then stop without proposing a consequential action.
- When the customer has made multiple explicit requests, plan actions for ALL of them. A terse ambiguous assent is not an explicit request for every open choice.
...
- For compensation, follow the same strict decision tree as support: create_refund for an explicitly requested full refund on one identified order, passing only order_id and reason because Shopify supplies the amount and currency; create_gift_card only for an explicitly merchant-directed fixed amount with a resolved customer_id; create_partial_refund for an explicitly requested item-only refund, naming each line item and quantity and never an amount. Unresolvable line items, missing identity, prior refunds, active disputes, and percentage discounts require merchant handling. Never invent compensation or substitute one form for another.
