...
export async function refreshTikTokShopAccessToken(
  config: TikTokShopTokenRefreshConfig,
  refreshToken: string,
): Promise<TikTokShopTokenResult> {
...
export async function sendTikTokShopTextMessage({
  ...
  let res: Response;
  try {
    res = await fetchWithDeadline(url.toString(), {
      cache: 'no-store',
      method: 'POST',
      headers: {
...
async function fetchWithDeadline(
  ...
  try {
    ...
    if (error instanceof Error && (error.name === 'AbortError' || error.name === 'TimeoutError')) {
      throw new TikTokShopRequestTimeoutError(error);
    }
    throw error;
  }
}

class TikTokShopRequestTimeoutError extends Error {
  constructor(cause: unknown) {
    super('TikTok Shop request timed out', { cause });
    this.name = 'TikTokShopRequestTimeoutError';
  }
}

function isRequestTimeoutError(error: unknown): error is TikTokShopRequestTimeoutError {
  return error instanceof TikTokShopRequestTimeoutError;
}
...
async function requestToken(
  ...
  let res: Response;
  try {
    res = await fetchWithDeadline(url.toString(), init);
  } catch (error) {
    if (isRequestTimeoutError(error)) {
