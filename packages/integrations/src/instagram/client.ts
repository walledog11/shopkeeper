...
/** @deprecated Use InstagramApiResult */
export type InstagramGraphResult<T> = InstagramApiResult<T>;
...
/** @deprecated Use InstagramLongLivedToken */
export type InstagramRefreshedToken = InstagramLongLivedToken;
...
export function fetchConnectedInstagramAccount(
  accessToken: string,
): Promise<InstagramApiResult<InstagramConnectedAccount>> {
  const url = new URL(`${INSTAGRAM_GRAPH_BASE_URL}/me`);
  url.searchParams.set('fields', 'user_id,username,account_type');
...
export function fetchInstagramMessagingUserProfile(
  igsid: string,
  accessToken: string,
): Promise<InstagramApiResult<InstagramMessagingUserProfile>> {
