...
**Implemented 2026-08-23; paid gate remains open.** `planner-tool-selection.ts` uses aligned classifier intents, fails open to the full set for missing/stale/ambiguous classifications, and leaves operator/storefront/merchant-answer paths unchanged. Narrowed buckets carry the three required control tools. A planning-only typed widening signal plus empty/incomplete-plan detection permits one clean judgment-tier retry against the full set. Structured completion logs expose the original bucket and a `namespaceMiss` boolean/reason for rate aggregation. Deterministic schema measurements show −67% for order-status and −81% for policy buckets, including the widening tool.

[The first exact-SHA release attempt](https://github.com/walledog11/shopkeeper/actions/runs/32627931331) on `f5f465fb6e39797a97c9cc401f91b872f41130c9` is not accepted: free preflight and the gateway hard case passed, but the dashboard finished at 46/49 hard-gated attempts. One explicit-refund fixture selected `create_return` twice, and the last fixture then crossed the dashboard's $0.6375 allocation at $0.6633 / 96 calls even though total spend remained inside the $0.75 authorization. The correction makes explicit full-refund language outrank an inferred return and reallocates the unchanged total ceiling to $0.70 dashboard / $0.05 gateway; the successful gateway used only $0.0088 / 2 calls. Internal-audit 5.2 remains open pending a newly authorized release run on the corrected SHA.
...
| 5 | Cost & housekeeping | internal audit | 6 recommended | **5.2 implemented; first exact-SHA paid gate diagnosed and corrected, replacement authorization pending.** Includes §1.1 typed namespace-miss fallback; 5.3 remains open. |
