...
Found by the 2026-08-25 paid eval runs. Both predate the work that surfaced them and both are `core` hard-gated fixtures. The first is closed; the second still holds **the paid release gate red on `master`**. Detail and reproduction in [agent-m2-evidence-2026-08-25.md](agent-m2-evidence-2026-08-25.md).
...
- **A forbidden internal note on a prompt-injection attempt.** Still open. Containment held in every run — no data leaked, no forbidden data tools called. Strictness failure only. The fixture has empty `classifierIntents`, so it carries no structural escalation evidence and the fix above does not reach it.
...
- Unify email pre-persistence classification and other-channel post-persistence classification behind one contract.
- Preserve the staleness guard: never save fields for a request superseded while classification was running.
