...
**Complete 2026-08-23 with paid split-run evidence.** `planner-tool-selection.ts` uses aligned classifier intents, fails open to the full set for missing/stale/ambiguous classifications, and leaves operator/storefront/merchant-answer paths unchanged. Narrowed buckets carry the three required control tools. A planning-only typed widening signal plus empty/incomplete-plan detection permits one clean judgment-tier retry against the full set. Structured completion logs expose the original bucket and a `namespaceMiss` boolean/reason for rate aggregation. Corrected deterministic schema measurements show −67.6% for order-status and −81.4% for policy buckets, including the widening tool.

[The first release attempt](https://github.com/walledog11/shopkeeper/actions/runs/32627931331) on `f5f465fb6e39797a97c9cc401f91b872f41130c9` passed free preflight, the gateway hard case, and 46 distinct dashboard fixtures. One explicit-refund fixture selected `create_return`, and the final fixture crossed the dashboard's $0.6375 allocation at $0.6633 / 96 calls. The correction makes explicit full-refund language outrank an inferred return and reallocates the unchanged release ceiling to $0.70 dashboard / $0.05 gateway; the successful gateway used $0.0088 / 2 calls.

Per the user's explicit instruction not to rerun paid cases that had already passed, [targeted run 32628935184](https://github.com/walledog11/shopkeeper/actions/runs/32628935184) tested only `tier-guarded-refund-approval` and `tier-watch-refund-draft-only` on corrected SHA `491f83ed90de5cc7429d0761857de51e5f0f917a`. Both passed (hard-gated 2/2, 100.0%) for $0.0892 / 6 calls under a $0.10 / 12-call cap; free preflight passed, and all full-release/drift/gateway jobs were skipped. The accepted gate is intentionally composite evidence—46 earlier dashboard passes, two corrected targeted passes, and the earlier gateway pass—not a repurchased full-suite run on one SHA.
...
| 5 | Cost & housekeeping | internal audit | 6 recommended | **5.2 complete with deterministic coverage and user-directed paid split-run evidence.** Includes §1.1 typed namespace-miss fallback; 5.3 remains open. |
