...
- [x] **Finish dashboard noindex coverage.** `apps/dashboard/next.config.js` now ships `X-Robots-Tag: noindex, nofollow` for `/dashboard`, `/api` (covers integration OAuth callbacks under `/api/integrations/*/callback`), and the auth + onboarding paths (`/login`, `/signup`, `/select-org`, `/create-org`, `/welcome`, `/plan`, `/connect`). `robots.txt` remains as the secondary signal.
- [x] **Add Telegram per-`chatId` flood limiting.** `webhooks-telegram.ts` now calls the shared `rateLimit` helper with key `webhook:telegram:{chatId}` (30 messages / 60s window) right after signature verification — applies to bound and unbound chats. On overage the request is logged and dropped silently with a 200 (so Telegram doesn't retry); no outbound reply, so the limiter can't be used as an amplifier. Covered by 3 tests in `webhooks-telegram.test.ts`.
...
- [x] **Add per-provider forwarding instructions in `IntegrationCard.tsx`.** `EmailForwardingDisclosure` now has a provider picker (Google Workspace / Outlook 365 / cPanel / Cloudflare) with step-by-step instructions for each. Verification flows (Gmail code, Cloudflare link) explicitly note that the verification email lands as a ticket in Clerk so merchants know where to look.
