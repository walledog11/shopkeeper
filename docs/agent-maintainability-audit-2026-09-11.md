...
[AGENT_AUDIT.md](../AGENT_AUDIT.md) records prior premature completion claims: approval-path replanning failures, a missing migration stopping all planning, and fixture/prompt disagreements. Its closed milestone ledger is historical evidence, not current product certification. The operating instructions also admit prompt growth while prohibiting special-case growth. More rules in a document have not eliminated this failure mode.
