...
Created: 2026-09-07. Last reconciled: 2026-09-10 (third pass, after the fixes the second pass called pending landed). Status: in progress; A1 and A2 are verified, A4's implementation is landed, B2 is in progress, and Gate 1 remains open. **A3 is the critical path.** On 2026-09-10 a real Instagram DM naming an order produced a mutative plan and that plan reached the merchant's phone as an approval card — which is where the proven part stops. **The plan the merchant approved never ran.** A forensic pass over the production rows that evening found `executionId` null on every action and no `PlanExecution` claim: the parked plan had been discarded before the merchant's reply arrived, `approve_pending_plan` correctly reported nothing to approve, and the model then attempted the refund on its own authority. It was stopped by a Shopify currency guard, not by anything in the approval path. Everything after "the card reaches the phone" is therefore unproven, and the earlier reading of this run — that it executed as `human_approved` — was an artifact of that label being the default for an unstated turn. The three fixes landed as `afc88439` ([PR #89](https://github.com/walledog11/shopkeeper/pull/89)) and are deployed. **One of them did not hold live.** A 2026-09-11 re-run escalated with the same `currency_mismatch` — "requested currency USD does not match Shopify currency CAD" — because the real cause was never the field the guard read. `refunds/calculate.json` was called without naming a currency, so Shopify priced the refund in the shop's own currency (CAD) and the code then compared that answer to the currency the customer was actually charged (USD). `afc88439` changed which of two shop-currency fields it read, and its regression stubbed the calculation as already answering in the presentment currency, so the test agreed with the code and the provider did not. The currency is now resolved from the order and passed *into* the calculation, in both refund paths. **What remains for the leg is the re-run itself, which is a live-provider task and not a code one.** The SocialAPI merchant connect has still never run for a live merchant, and is closed by configuration in production.
