...
- [ ] **5.2** Intent-driven tool selection (prior item 8). **27 schemas now ship on every iteration** — `TOOL_DEFINITIONS` holds 31, but `AGENT_TOOLS` filters to `availability === "active"` (`registry/index.ts`) and `AGENT_TOOLS` is what reaches the API. The "(28 when this was written)" parenthetical this item used to carry had it exactly backwards: 28 was right for the pre-Phase-6 tree, and re-counting the wrong array turned it into 30. Serialized, the 27 are **22,997 characters ≈ 5,749 tokens** at 4 chars/token (`JSON.stringify(AGENT_TOOLS).length` against the built package); the `~6,926` quoted before has no recorded method.

  **Phase 6 moved this number the wrong way, and that was the trade.** 6.2 removed a schema and 6.3 lengthened five descriptions, netting +1,179 characters ≈ **+295 tokens** against the pre-Phase-6 21,818/5,455. Tool schemas sit in the cached prefix, so the marginal cost is cache-read tokens rather than fresh input, and what it buys is the boundary between refund / return / exchange / cancel / edit stated where the model reads it. Size 5.2 against 22,997, not against the old figure. Verified: nothing in `run.ts`, `planner.ts`, or `prompt.ts` reads `classifierSignals` for tool selection, and the classifier's `intents` are already on `ctx.classifierSignals` (`classifier-signals.ts:194`). **1–2 days. High risk** — a wrongly-narrowed set produces "I can't help with that" instead of an action, and it changes the cached prefix per intent bucket. Measure before committing. **Gate on the full suite.**
