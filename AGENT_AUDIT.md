...
- `endSentence` — `planning-notifications.ts:163` trims first; `digest.ts:238` does not, producing `"text ."`. *(The `digest.ts` copy was gone before 5.4 ran; one implementation, the trimming one.)*
- `lowerFirst` — **not duplicated verbatim, which this list had backwards.** `digest-briefing.ts:930` lowercases unconditionally; `planning-notifications.ts:157` only lowercases when the string matches `/^[A-Z][a-z]/`. So an acronym opener diverges by surface: "URL expired" stays itself on the operator card and reaches the briefing as "uRL expired". It is the most drifted of the three, not the safe one. *(One implementation now — `apps/gateway/src/lib/sentence-case.ts`, the guarded one. Phase 5.4.)*
...
It also fixes the product problem the live probe exposed: with `deadline` as a field, the briefing can lead with *"Customer deadline: Fri, Aug 28, 2026"* instead of burying the timing 180 characters into a sentence. Without that field the briefing cannot know which part of the sentence matters; the first live phone round-trip also established that a weekday alone is ambiguous.
...
| 2 | Validate, don't repair (C) | ~2 d | −150 | Yes | **Complete 2026-08-22** — 44/44 dashboard fixtures plus the gateway hard case passed |
| 3 | One autonomy function (B) | ~3 d | −400 | Yes | **Complete 2026-08-22** — 44/44 dashboard fixtures plus the gateway hard case passed |
| 4 | Structured rendering (D) | 8–10 d | −1,500 | Yes + phone | **Complete 2026-08-22** — live delivery confirmed; feedback corrections landed and passed deterministic coverage |
| 5 | Cost & housekeeping | ~3 d | −350 (unmeasured) | 5.2 only | Open — 5.1 closed, 5.1a struck, 5.4 closed; 5.2 and 5.3 re-sized after re-verification. Nothing here gates Phase 3 |
...
- [x] **2.5 Gate.** This changes what reaches the customer on plans the model got wrong — exactly what the fixtures grade. **Completed 2026-08-22.**

**2026-08-22 release evidence:** the exact-SHA paid run on `4cd07169b57d1c13ed024536418ab649abfa0409` passed all 44 dashboard fixtures and the independent gateway hard case. It used 86 model calls and $0.5108 of the approved 120-call/$0.75 ceiling. The workflow's final status was a reporting false negative: the gateway summary validator anchored its `grep` at byte zero, but Vitest prefixed the passing gate and budget lines with ANSI color bytes. The artifact records `clear-fraud 1/1`, `$0.0082`, and two calls. The validator is fixed on the PR. **Decision 2026-08-22: accept the passing behavior assertions and artifact as the gate result; do not pay to rerun successful model behavior solely to turn orchestration metadata green.** [Run evidence](https://github.com/walledog11/shopkeeper/actions/runs/32618869853).

**Done when:** `planner.ts` contains one validation call, not six mutation passes. **Met.** Unit, typecheck, lint, database-backed integration suites, and the paid behavior gate pass.
...
- [x] **3.6 Gate.** Run the budgeted release profile (44 hard core fixtures plus the independent gateway hard case). The complete 84-fixture, three-repeat profile remains the non-release drift measurement; advisory results cannot fail release. **Completed 2026-08-22.**

The 2026-08-22 paid behavior evidence passed 44/44 dashboard fixtures and gateway clear-fraud 1/1 on exact SHA `4cd07169b57d1c13ed024536418ab649abfa0409`. Per the 2.5 evidence decision, that passing artifact closes 3.6; the ANSI-sensitive reporting false negative is not a model-behavior failure and does not justify another paid run.
...
**Local and release status (2026-08-22):** the full repository suites, package typechecks, lint, builds, structure checks, and `git diff --check` pass. The paid run described above passed every behavior assertion. Phase 3 is complete.
...
- [x] **4.4** Delete the tense engine (`humanizeReportedSummary`, `REPORTED_VERB_PAST`, `REPORTED_SPEECH`, `SUMMARY_PREAMBLE`), `tidyPunctuation`, and the truncation cascade. *(Completed 2026-08-22: those symbols are gone; structured facts/`RequestDisplay` own phone lines, source-verbatim short quotes remain bounded and contact-redacted, and the old multi-budget cascade collapsed to a general phone-line bound.)* The implementation resolved two historical blockers:
  ...
  - *`ask: "none"`.* `formatFactsBriefingLine` returns null when `!parts.ask && !parts.deadline` (`briefing-fields.ts:121`), and `ask: "none"` is a legitimate **v5** output. This item read that as a rendering decision — what should an ask-less line say — because the existing-customer email fast path skipped the classifier and wrote `requestFacts: emptyRequestFacts()`, so **every repeat customer emailing in got `ask: "none"`** permanently: a precomputed result also sets `skipSummary` (`inbound-persistence.ts:339`), so nothing filled the fields in later. **That was a bug, not a decision, and it is fixed** — the fast path now classifies and overrides only the spam verdict, so the fields land and the placeholder survives only for a failed classification.
    ...
    Why it was there is worth recording, because the shape recurs. The bypass is a locked decision in `docs/spam-filter-plan.md` from `9511c69d` (2026-04-28): *"if the sender has any prior `filterStatus = genuine` thread, skip the classifier entirely"*, justified as *"save a call previously paid on every spammer."* Correct at the time — the classifier answered exactly one question, and for a known-good sender it was already answered. It then picked up `intents`, `requestSummary`/`requestDisposition` (v4) and `requestFacts` (v5), and each addition wrote a **placeholder** into the fast path instead of reopening the bypass. `a3132387`'s own commit message shows the author looking straight at it — *"the disposition falls back to `unclear` rather than `none` — including on the existing-customer fast path that skips the classifier"* — and choosing the fail-safe default. Three times, reasonably, on the data side where the prompt rule about growth-by-special-case was not looking.
...
- [x] **4.7 Gate**, plus a live phone round-trip: operator copy is verified by phone, not by evals. **Completed 2026-08-22.**

**Live-phone provider delivery and recipient review completed 2026-08-22.** The iMessage card arrived, and the round-trip did what this gate was designed to do: it exposed that masking the street/unit/ZIP made an address-change approval unactionable and that `By Friday` did not say whose deadline it was or identify the calendar date. Authenticated operator surfaces now retain the full actionable postal address, while unrelated contact-detail protections remain. Resolved deadlines render as an explicit label and date, for example `Customer deadline: Fri, Aug 28, 2026`. Commit `3b4f3089` covers the corrected card, question, auto-execution, ledger, and digest surfaces; 379 gateway unit tests and 846 gateway integration tests passed locally, including focused renderer regressions. A second live send is not required to repeat an already-successful provider-delivery test for deterministic copy changes. The local Railway runner's inability to reach the private Redis hostname affected only post-send idempotency bookkeeping, and the card was not resent.

**Done when:** `digest-briefing.ts` contains no regex over model-written prose. **Met 2026-08-22.**

**Decisions:**
- ~~What does a briefing line lead with when a ticket has several asks?~~ **Decided 2026-08-21: the deadline. Refined by the live phone test on 2026-08-22:** a resolved deadline must identify itself and include the full calendar date (`Customer deadline: Fri, Aug 28, 2026`), never only `By Friday`.
...
- [x] ~~Redact full postal addresses from phone notifications by default.~~ **Reversed after the live phone test on 2026-08-22.** The merchant is the authenticated operator, and address-change cards are not actionable without the exact street, unit, and postal code. Cards, drafts, plan steps, escalation reasons, questions, auto-execution notices, ledgers, and digests now retain actionable postal addresses; unrelated email/phone contact protections remain.
...
- [x] **5.2** Intent-driven tool selection (prior item 8). **Complete 2026-08-23 with paid split-run evidence.** `planner-tool-selection.ts` now selects from the classifier's aligned boolean intent contract; operator turns, storefront security allowlists, merchant-answer replans, missing/stale classifier state, and unclassified requests retain their prior full set. Every narrowed customer bucket contains `send_reply`, `escalate_to_human`, and `ask_operator` (plus the existing thread bookkeeping tools). Risk intents dominate to a control-only bucket; simultaneous non-risk intents union their tool sets. Mutations stay deliberately broad so the Phase 6.3 adjacent-action fixtures see every competing mutation. `RequestFacts` remains renderer-only rather than becoming an ungraded second routing contract.

  A planning-only `request_wider_tool_set` signal, an empty plan, or a non-terminal/incomplete plan triggers **one** clean retry against the full enabled registry on the judgment tier. The signal is not in the executable registry and cannot survive into a cached plan. Every completion log carries `toolSelectionBucket`, narrowing/count fields, and `namespaceMiss`; the dedicated miss log carries the original bucket and typed miss reason, so `namespace_miss` rate is directly aggregable without matching prose.

  Re-derived against the corrected built tree: the 27 active schemas are **23,357 characters ≈ 5,839 tokens** (`JSON.stringify(AGENT_TOOLS).length`). Including the widening-control schema, representative narrowed sets are: risk/no-request 3,856 chars (~964 tokens, −83.5%); policy 4,354 (~1,089, −81.4%); order status 7,570 (~1,893, −67.6%); coarse order mutation 18,612 (~4,653, −20.3%). These are deterministic schema-size comparisons, not observed bill savings; real savings depend on bucket mix, cache reads, iterations, and namespace misses.

  Free evidence: 838 agent unit tests, 70 agent integration tests, 379 gateway unit tests, 843 gateway integration tests (+1 skipped), 753 dashboard unit tests, 660 dashboard integration tests (+3 skipped), 60 repository script tests, repository typecheck/lint/structure/knip, eval-control tests, fixture loading, and the gateway deterministic pre-filter pass. Release budget preflight estimates **$0.6598 / 87 calls** against the standing $0.75 / 120 ceiling.

  **Paid evidence, 2026-08-23:** [run 32627931331](https://github.com/walledog11/shopkeeper/actions/runs/32627931331) on `f5f465fb6e39797a97c9cc401f91b872f41130c9` passed the free preflight, gateway hard case (1/1, $0.0088 / 2 calls), and 46 distinct dashboard fixtures. `tier-guarded-refund-approval` chose `create_return` on its initial attempt and confirmations despite an explicit full-refund request; the final `tier-watch-refund-draft-only` attempt then crossed the dashboard's overly tight sub-allocation at $0.6633 / 96 calls. The correction makes an explicit full-refund request outrank an inferred return and reallocates the unchanged release ceiling to $0.70 dashboard / $0.05 gateway based on the preserved spend evidence.

  At the user's explicit direction, the 46 passing fixtures and passing gateway were not repurchased. [Targeted run 32628935184](https://github.com/walledog11/shopkeeper/actions/runs/32628935184) tested only the two prior dashboard failures on corrected exact SHA `491f83ed90de5cc7429d0761857de51e5f0f917a`: both passed, hard-gated 2/2 (100.0%), for $0.0892 / 6 calls under a $0.10 / 12-call cap. The free deterministic preflight also passed. Acceptance is therefore intentionally split evidence rather than a second full-suite single-SHA run: 46 prior dashboard passes + 2 corrected targeted passes + the prior gateway pass, with no already-passing paid case rerun.

  Historical sizing retained below for why this work was scheduled: **27 schemas shipped on every iteration** — `TOOL_DEFINITIONS` holds 31, but `AGENT_TOOLS` filters to `availability === "active"` (`registry/index.ts`) and `AGENT_TOOLS` is what reaches the API. The "(28 when this was written)" parenthetical this item used to carry had it exactly backwards: 28 was right for the pre-Phase-6 tree, and re-counting the wrong array turned it into 30. The `~6,926` tokens quoted before has no recorded method.

  **Phase 6 moved this number the wrong way, and that was the accepted trade.** Against the recorded pre-Phase-6 21,818 characters, the corrected current registry's 23,357 is +1,539 characters ≈ **+385 tokens**: 6.2 removed a schema, 6.3 lengthened five descriptions, and the paid-gate correction made the refund/return boundary explicit. Tool schemas sit in the cached prefix, so the marginal cost is cache-read tokens rather than fresh input, and what it buys is the boundary between refund / return / exchange / cancel / edit stated where the model reads it. The implementation above now reads `ctx.classifierSignals`; before it, no planner/model/prompt code used those signals for tool selection. This was a **high-risk** gate because a wrongly-narrowed set can produce "I can't help with that" instead of an action and changes the cached prefix per intent bucket; the deterministic fallback coverage and paid split-run evidence above close it.
...
- [x] **5.4** Deduplicate: ~~`customerFirstName`~~ (done in 4.5), `endSentence`, `lowerFirst`, `fallbackTitleFromSummary` (×2, drifted), `isDeterministicE2EAIEnabled` (×2, different `NODE_ENV` semantics). ~~Delete `tools/tool-inputs.ts` (23 LOC, unimported).~~ **Wrong — it is imported**, type-only, by `agent-context.ts:9` for five tool input types. Nothing to delete. **Closed 2026-08-23.**

  - `endSentence` **needed nothing** — the `digest.ts:238` copy this item listed was already gone when the item was executed, leaving one definition at `planning-notifications.ts`. A dedupe list carried across three passes accumulates entries that other work has already closed; check each one before writing the diff, not after.
  - `lowerFirst` → `apps/gateway/src/lib/sentence-case.ts`, keeping the **guarded** version. §2.5 had this backwards twice over: it is the drifted one, and the digest's unguarded copy was the wrong one to keep. "URL expired" now reads the same on the operator card and in the briefing.
  - `fallbackTitleFromSummary` → `packages/agent/src/classifier-signals.ts`, where the title contract already lives. The two copies stripped different lead-in verbs and truncated to the same 70 characters with different ellipses, so one summary produced two titles depending on which surface asked. The surviving one takes the union of the lead-ins.
  - `isDeterministicE2EAIEnabled` → the existing export in `packages/agent/src/ai/index.ts`; the gateway classifier's copy is deleted. The semantics gap was real but inert: `with-test-env.mjs:46` pins `NODE_ENV` to `test`, and nothing else sets `E2E_AI_MODE`, so `=== "test"` and `!== "production"` never disagreed in any harness. Production is the boundary worth guarding, so the exported predicate wins unchanged.
- [x] **5.5** ~~Remove the two prompt lines that duplicate code-enforced rules (`prompt.ts:195`, `:200`)~~ — **one of the two, not both.** Collapse the three restatements of the `get_order_tracking` rule (verified: 3 bullets). **Closed 2026-08-23**, with `:195` deliberately still there: `:200` is deleted, and the three support-side restatements are one. The middle bullet repeated `get_order_tracking`'s own description almost word for word and is gone; the other two keep only what the description does not say (prefer the order data already in context; if tracking comes back empty, still answer from the order's status). The **operator** branch keeps its two restatements — operator prompt changes are verified by phone, not by evals, and this change owed no phone round-trip. `prompting.test.ts` now asserts the support prompt does *not* restate the rule, so the third copy cannot grow back.
...
- *Phase 1* — holds exactly. Nine `ProducedPlanSignalCode`s plus `legacy_warning`, and
  ...
  hits repo-wide.
- *Phases 2 and 3* — complete 2026-08-22; database-backed and full repository suites pass, and the paid behavior evidence passed. The ANSI-summary parser failure was a reporting false negative and does not leave either phase open.
- *Phase 4* — complete 2026-08-22. Operator cards, draft summaries, pending-plan ledgers, and digests consume structured `RequestDisplay`; the prose fallback and its fixtures are gone. The live receipt exposed address masking and ambiguous relative-deadline copy, both corrected and covered by deterministic renderer tests.
