...
| 4 | Structured rendering (D) | 8–10 d | −1,500 | Yes + phone | **Live feedback corrections in progress 2026-08-22** — provider delivery confirmed; corrected phone copy + clean conclusive model workflow remain |
...
**Live-phone provider delivery confirmed 2026-08-22, and the round-trip rejected two earlier renderer decisions.** The iMessage card arrived, but masking the street/unit/ZIP made an address-change approval unactionable, and the lead `By Friday` did not say whose deadline it was or identify the calendar date. Authenticated operator surfaces now retain the full actionable postal address, while unrelated contact-detail protections remain. Resolved deadlines now render as an explicit label and date, for example `Customer deadline: Fri, Aug 28, 2026`. Keep 4.7 unchecked until that corrected phone copy is verified and the model workflow returns a clean conclusive result. The original send still proved the production provider binding; the local Railway runner's inability to reach the private Redis hostname affected only post-send idempotency bookkeeping, and the card was not resent.
...
- *Phase 4* — 4.1–4.6 and provider delivery hold. Operator cards, draft summaries, pending-plan ledgers, and digests consume structured `RequestDisplay`; the prose fallback and its fixtures are gone. The first live receipt exposed address masking and ambiguous relative-deadline copy, both now corrected in the renderer. 4.7 waits for verification of the corrected phone copy and the clean conclusive model workflow.
