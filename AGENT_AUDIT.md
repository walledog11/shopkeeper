...
- [ ] **5.2** Intent-driven tool selection (prior item 8). **Implementation complete locally 2026-08-23; remains open until the exact-SHA paid release gate passes.** `planner-tool-selection.ts` now selects from the classifier's aligned boolean intent contract; operator turns, storefront security allowlists, merchant-answer replans, missing/stale classifier state, and unclassified requests retain their prior full set. Every narrowed customer bucket contains `send_reply`, `escalate_to_human`, and `ask_operator` (plus the existing thread bookkeeping tools). Risk intents dominate to a control-only bucket; simultaneous non-risk intents union their tool sets. Mutations stay deliberately broad so the Phase 6.3 adjacent-action fixtures see every competing mutation. `RequestFacts` remains renderer-only rather than becoming an ungraded second routing contract.
  ...
  Re-derived against the built implementation tree: the 27 active schemas are **22,956 characters ≈ 5,739 tokens** (`JSON.stringify(AGENT_TOOLS).length`). Including the widening-control schema, representative narrowed sets are: risk/no-request 3,856 chars (~964 tokens, −83.2%); policy 4,354 (~1,089, −81.0%); order status 7,570 (~1,893, −67.0%); coarse order mutation 18,211 (~4,553, −20.7%). These are deterministic schema-size comparisons, not observed bill savings; real savings depend on bucket mix, cache reads, iterations, and namespace misses.
  ...
  **First paid evidence, 2026-08-23:** exact SHA `f5f465fb6e39797a97c9cc401f91b872f41130c9` did not pass and is not accepted. [Run 32627931331](https://github.com/walledog11/shopkeeper/actions/runs/32627931331) passed the free preflight and gateway hard case (1/1, $0.0088 / 2 calls), while the dashboard recorded 46/49 hard-gated attempts. `tier-guarded-refund-approval` chose `create_return` on both the initial attempt and two confirmation runs despite an explicit full-refund request, then the final fixture crossed the dashboard's overly tight sub-allocation at $0.6633 / 96 calls. Follow-up wording makes an explicit full-refund request outrank an inferred return, and the unchanged $0.75 total ceiling is reallocated to $0.70 dashboard / $0.05 gateway based on the preserved spend evidence. **Still owed:** a newly authorized `workflow_dispatch mode=release` on the corrected final SHA; do not mark this checkbox closed before all 48 core fixtures and the gateway hard case pass.
  ...
  **Phase 6 moved this number the wrong way, and that was the accepted trade.** Against the recorded pre-Phase-6 21,818 characters, the current 22,956 is +1,138 characters ≈ **+285 tokens**: 6.2 removed a schema and 6.3 lengthened five descriptions. Tool schemas sit in the cached prefix, so the marginal cost is cache-read tokens rather than fresh input, and what it buys is the boundary between refund / return / exchange / cancel / edit stated where the model reads it. The implementation above now reads `ctx.classifierSignals`; before it, no planner/model/prompt code used those signals for tool selection. This remains **high risk** until the paid gate: a wrongly-narrowed set can produce "I can't help with that" instead of an action, and it changes the cached prefix per intent bucket.
