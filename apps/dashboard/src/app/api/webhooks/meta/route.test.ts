...
beforeEach(() => {
  mockFetch.mockReset();
  vi.stubEnv('INSTAGRAM_WEBHOOK_APP_SECRET', 'parent-meta-secret');
  vi.stubEnv('INSTAGRAM_APP_SECRET', 'instagram-oauth-secret');
});
...
describe('/api/webhooks/meta forwarding', () => {
  ...
  it('validates and forwards the exact signed body', async () => {
    ...
    const signature = `sha256=${createHmac('sha256', 'parent-meta-secret').update(body).digest('hex')}`;
