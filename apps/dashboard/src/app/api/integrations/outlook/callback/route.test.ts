...
describe('GET /api/integrations/outlook/callback', () => {
  it('rejects state mismatch before token exchange', async () => {
    ...
    const res = await GET(new Request('http://localhost/api/integrations/outlook/callback?code=abc&state=other_state'));
  ...
  it('rejects user session mismatch', async () => {
    ...
    const res = await GET(new Request('http://localhost/api/integrations/outlook/callback?code=abc&state=state_123'));
  ...
  it('persists outlook integration and removes any other email rows for the org', async () => {
    ...
    const res = await GET(new Request('http://localhost/api/integrations/outlook/callback?code=oauth_code&state=state_123'));
  ...
  it('redirects to access_denied when user cancels', async () => {
    const res = await GET(new Request('http://localhost/api/integrations/outlook/callback?error=access_denied'));
    expect(res.status).toBe(307);
