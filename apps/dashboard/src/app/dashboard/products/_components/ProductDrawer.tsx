...
import type { ProductRow } from "./products-page-utils"
...
export function ProductDrawer({ product, isOpen, onClose, shop }: {
  ...
  return (
      ...
      <button type="button" aria-label="Close product drawer" className="absolute inset-0 border-0 bg-neutral-950/50 p-0 sm:bg-neutral-950/30" onClick={onClose} />
      <div
        className={`
        ...
        `}
      >
        <ProductDrawerContent product={product} shop={shop} onClose={onClose} />
      </div>
    </div>
  )
}
