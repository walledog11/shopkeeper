  ...
  return (
    <div className="flex flex-col sm:flex-row h-full overflow-hidden">
      ...
      <aside className="hidden sm:flex flex-col w-[220px] shrink-0 border-r border-white/[0.07] bg-background overflow-y-auto py-5 px-3">
        <div className="px-2.5 pb-2.5 text-[10px] font-semibold tracking-[0.06em] uppercase text-white/40">
          Settings
        </div>
        <nav className="flex flex-col gap-0.5">
          {NAV_ITEMS.map(({ id, label, icon: Icon, hint }) => {
            ...
            return (
              ...
              >
                <Icon className={cn("w-4 h-4 shrink-0 mt-0.5", active ? "text-amber-400" : "text-white/40")} />
