...
function highlightVars(text: string) {
  ...
  return text.split(/({{[^}]+}})/).map((part) => {
    ...
    offset += part.length
    return /^{{[^}]+}}$/.test(part)
      ? <mark key={key} className="bg-violet-400/15 text-violet-300 rounded px-0.5 not-italic font-mono text-[0.9em]">{part}</mark>
    ...
    <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] hover:bg-white/[0.04] hover:border-white/[0.11] transition-all overflow-hidden">
      <div className="flex gap-3 p-4">
        ...
        <div className="flex items-center gap-0.5 shrink-0 self-start">
          <button type="button"
            onClick={handleCopy}
            className={`flex flex-col items-center gap-1 text-xs font-medium px-3 py-2 rounded-lg transition-all ${
              copied
          ...
          <button type="button"
            onClick={onEdit}
            className="flex flex-col items-center gap-1 text-xs font-medium text-white/35 hover:text-white/80 hover:bg-white/[0.08] px-3 py-2 rounded-lg transition-all"
          >
          ...
          <button type="button"
            onClick={onDelete}
            className="flex flex-col items-center gap-1 text-xs font-medium text-white/35 hover:text-red-400 hover:bg-red-400/10 px-3 py-2 rounded-lg transition-all"
          >
