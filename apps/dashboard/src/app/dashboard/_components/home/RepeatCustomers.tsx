  ...
  return (
    <div className="flex flex-col gap-2.5">
      <div className="flex items-baseline justify-between gap-2">
        <p className="text-xs uppercase tracking-wider font-semibold text-white/40">Repeat customers</p>
        <p className="text-xs text-white/35">contacted 3+ times in 30d</p>
      </div>
      <div className="rounded-md bg-card border border-border overflow-hidden divide-y divide-white/[0.04]">
        {customers.map((c, i) => (
          ...
          >
            <div className={`size-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${COLORS[i % COLORS.length]}`}>
              {c.initials}
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-[12px] font-semibold text-white/85 truncate">{c.name}</p>
              <p className="text-xs text-white/35 truncate">{c.ticketCount} tickets</p>
            </div>
          </Link>
        ))}
      </div>
