...
export function TicketStackDeck({
  ...
  const nextItem = n > 1 ? tickets[mod(activeIndex + 1)] : null
  const isCurrentActive = Boolean(current && activeTicketId === current.id)

  useEffect(() => {
    if (!activeTicketId || !tickets.some(ticket => ticket.id === activeTicketId)) return
    setCurrentId(activeTicketId)
  }, [activeTicketId, tickets])

  useEffect(() => {
    const controls = animate(stackDragX, 0, { type: "spring", stiffness: 420, damping: 34 })
  ...
  const commitNeighbor = (delta: 1 | -1) => setCurrentId(tickets[mod(activeIndex + delta)].id)
