...
export function AnalyticsOverviewPanel({ preset, rangeFrom, rangeTo }: Props) {
  const rangeDays = Math.max(
    1,
    Math.ceil((rangeTo.getTime() - rangeFrom.getTime()) / (1000 * 60 * 60 * 24)),
  )
  ...
  const openCount =
    (data?.threads.byStatus["open"] ?? 0) + (data?.threads.byStatus["pending"] ?? 0)
  ...
  const avgMessages =
    totalThreads > 0 ? Math.round((totalMessages / totalThreads) * 10) / 10 : 0
  ...
  const resolutionScore =
    totalThreads === 0
      ...
      : resolutionRate >= 80
        ? 100
        : resolutionRate >= 60
          ? Math.round(60 + ((resolutionRate - 60) / 20) * 40)
  ...
  const aiScore =
    totalReplies === 0
      ...
      : aiUsageRate >= 50
        ? 100
        : aiUsageRate >= 30
          ? Math.round(60 + ((aiUsageRate - 30) / 20) * 40)
  ...
  const msgScore =
    totalThreads === 0
      ...
      : avgMessages <= 4
        ? 100
        : avgMessages <= 6
          ? Math.round(60 + ((6 - avgMessages) / 2) * 40)
    ...
    avgResponseMinutes === null
      ...
      : avgResponseMinutes <= 30
        ...
        : avgResponseMinutes <= 240
          ? Math.round(60 + ((240 - avgResponseMinutes) / 210) * 40)
  ...
  const kpiCards = [
    ...
    },
    {
      ...
      barPct:
        totalThreads > 0
          ? Math.max(0, Math.min(100, 100 - ((avgMessages - 4) / 8) * 100))
          : 0,
    ...
    },
    {
      ...
      sub:
        avgResponseMinutes !== null
          ? `${firstReplyCount} tickets measured`
          : "No data yet",
      ...
      barPct:
        avgResponseMinutes !== null
          ? Math.max(0, Math.min(100, 100 - ((avgResponseMinutes - 5) / 235) * 100))
          : 0,
  ...
  if (totalThreads === 0) {
    ...
    if (resolutionRate < 60) {
      auditTips.push({
        ok: false,
        text: `Resolution is ${resolutionRate}% , enable AI auto-replies to clear your backlog faster`,
        benchmark: "Healthy ≥ 80% · OK 60–80% · Needs work < 60%",
      })
    }
    if (totalReplies > 0 && aiUsageRate < 30) {
      auditTips.push({
        ok: false,
        text: `AI handles only ${aiUsageRate}% of replies , turn on AI drafts to scale your team`,
        benchmark: "Healthy ≥ 50% · OK 30–50% · Low < 30%",
      })
    }
    if (avgMessages > 6) {
      auditTips.push({
        ok: false,
        text: `${avgMessages} messages per ticket on average , add FAQs to your AI context to shorten threads`,
        benchmark: "Healthy ≤ 4 msgs · OK 4–6 · Too long > 6",
      })
    }
    if (avgResponseMinutes !== null && avgResponseMinutes > 240) {
      auditTips.push({
        ok: false,
        text: `${formatResponseTime(avgResponseMinutes)} average first reply , configure AI triage to respond instantly`,
        benchmark: "Healthy < 30 min · OK < 4h · Slow > 4h",
      })
    }
    if (auditTips.length === 0) {
      auditTips.push({
        ok: true,
        text: "All metrics are within healthy ranges , keep it up!",
        benchmark: "",
      })
    }
  }
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        <TicketChart
          ...
          isLoading={isLoading}
        />
        <TopTopicsCard byTag={byTag} maxTag={maxTag} isLoading={isLoading} />
      </div>

      {!isLoading && <ChannelBreakdown byChannel={byChannel} maxChannel={maxChannel} />}
    </div>
  )
}
