...
async function main() {
  ...
  for (const c of contexts) {
    ...
    console.log(`  ${c.memberKey}  (updated ${c.updatedAt.toISOString()})`);
    console.log(`    pendingPlans:    ${queued.length > 0
      ? queued.map((plan) => `thread ${plan.threadId} — "${plan.instruction ?? ''}"`).join('; ')
      : '—'}`);
