...
// than an omission. The `operator` channel is one durable thread per binding, so
// a boundary there would fragment the merchant's own conversation. `shopify` is
// merchant-side order/email-fallback traffic.
// Boundaries are specified for exactly the four customer-origin channels below;
// inventing one for the rest would be policy nobody asked for.
