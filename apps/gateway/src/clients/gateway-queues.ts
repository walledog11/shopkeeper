...
type GatewayQueueDefaults =
  ...
  | typeof AGENT_TASK_QUEUE_DEFAULTS
...
const DEFAULT_QUEUE_OPTIONS: Partial<Record<string, { defaultJobOptions: GatewayQueueDefaults }>> = {
  ...
  // A claimed legacy-runtime attempt is never retried by BullMQ: the durable
  // task lifecycle decides whether it is safe to continue.
  [QUEUE.AGENT_TASK]: { defaultJobOptions: AGENT_TASK_QUEUE_DEFAULTS },
