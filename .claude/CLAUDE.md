...
- **Land agent-path work through a pull request.** `evals.yml` triggers on `pull_request`, so a change pushed straight to `master` is never gated — it becomes a manual "run the eval gate" to-do, discharged later by a ~$2.60 local full-suite run instead of the ~$0.51 CI core gate. 31 of 34 agent-path commits between 2026-08-09 and 08-19 bypassed the gate this way, planner-behavior changes among them. That backlog, not CI, is what the eval bill was paying for.
